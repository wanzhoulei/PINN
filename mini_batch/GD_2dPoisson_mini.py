...
n_iter = 1000; gamma=1.99; BC_ratio = 1; Interior_ratio=1; lr=0.004
write_pickle = False
...
##do SGD
SGD_optimizer(PINN, lr, n_iter, BC_ratio=BC_ratio, Interior_ratio=Interior_ratio)
...
tracepath = "./results/2dpoisson_SGD_lr{}_{}iter_seed{}_f_{}_N{}_{}_gamma{}_BCRatio{}_IntRatio{}.csv".format(lr, len(PINN.loss_trace), 
    random_seed, str(func_RHS).split(' ')[1], N, layertostr(layers), gamma, BC_ratio, Interior_ratio)
np.savetxt(tracepath, np.array(PINN.loss_trace), delimiter=",")

#save the cpu time
tracepath2 = "./results/2dpoisson_SGD_lr{}_{}iter_seed{}_f_{}_N{}_{}_gamma{}_BCRatio{}_IntRatio{}_CPUtime.csv".format(lr, len(PINN.loss_trace), 
    random_seed, str(func_RHS).split(' ')[1], N, layertostr(layers), gamma, BC_ratio, Interior_ratio)
...
figpath = './results/2dpoisson_SGD_lr{}_{}iter_seed{}_f_{}_N{}_{}_solution_gamma{}_BCRatio{}_IntRatio{}.png'.format(lr, len(PINN.loss_trace), 
    random_seed, str(func_RHS).split(' ')[1], N, layertostr(layers), gamma, BC_ratio, Interior_ratio)
...
plt.title("SGD on 2d Poisson with PINN loss trace")
##save the convergence trace plot
pltpath = "./results/2dpoisson_SGD_lr{}_{}iter_seed{}_f_{}_N{}_{}_loss_gamma{}_BCRatio{}_IntRatio{}.png".format(lr, len(PINN.loss_trace), 
    random_seed, str(func_RHS).split(' ')[1], N, layertostr(layers), gamma, BC_ratio, Interior_ratio)
plt.savefig(pltpath, dpi=300)

if write_pickle:
    pickle_writepath = './results/2dpoisson_SGD_lr{}_{}iter_seed{}_f_{}_N{}_{}_PINN_gamma{}_BCRatio{}_IntRatio{}.dat'.format(lr, len(PINN.loss_trace), 
        random_seed, str(func_RHS).split(' ')[1], N, layertostr(layers), gamma, BC_ratio, Interior_ratio)
