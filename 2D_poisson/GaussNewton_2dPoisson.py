'''
This file performs the Gauss Newton Algorithm on the 2d Poisson Equation with PINN
...
tracepath = "./results/2dpoisson_GN_{}iter_seed{}_alpha{}_f_{}_N{}_{}_gamma{}.csv".format(len(PINN.loss_trace), 
...
#save the cpu time
tracepath2 = "./results/2dpoisson_GN_{}iter_seed{}_alpha{}_f_{}_N{}_{}_gamma{}_CPUtime.csv".format(len(PINN.loss_trace), 
    random_seed, str(alpha).replace('.','d'), str(func_RHS).split(' ')[1], N, layertostr(layers), gamma)
...
figpath = './results/2dpoisson_GN_{}iter_seed{}_alpha{}_f_{}_N{}_{}_solution_gamma{}.png'.format(len(PINN.loss_trace), 
    random_seed, str(alpha).replace('.','d'), str(func_RHS).split(' ')[1], N, layertostr(layers), gamma)
...
pltpath = "./results/2dpoisson_GN_{}iter_seed{}_alpha{}_f_{}_N{}_{}_loss_gamma{}.png".format(len(PINN.loss_trace), 
    random_seed, str(alpha).replace('.','d'), str(func_RHS).split(' ')[1], N, layertostr(layers), gamma)
plt.savefig(pltpath, dpi=300)

if write_pickle:
    pickle_writepath = './results/2dpoisson_GN_{}iter_seed{}_alpha{}_f_{}_N{}_{}_PINN_gamma{}.dat'.format(len(PINN.loss_trace), 
        random_seed, str(alpha).replace('.','d'), str(func_RHS).split(' ')[1], N, layertostr(layers), gamma)
